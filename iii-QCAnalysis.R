...
for(m in 1:length(sp.list)) {
  ...
  # Calculate the probability that tau is negative for each cell
  tau_prob0<- rowMeans(tau_samps1< 0)
  ...
  #Probabilites  
  # Initialize a matrix to store predicted counts
  #   
  # n_cells<-length(cells_with_counts)  
  # n_route<-length(unique(sp.data$RouteIdentifier))
  # predicted_counts <- matrix(NA, nrow = posterior_ss, ncol = n_cells)
  # 
  # # # Loop through each posterior sample
  # for (i in 1:posterior_ss) {
  #   # Extract fixed effects and random effects
  #   fixed_effects <- samp1[[i]]$latent[1]  #nstop random effect
  #   random_effects_alpha <- samp1[[i]]$latent[2:(n_cells + 1)]  # ICAR random intercepts
  #   random_effects_tau <- samp1[[i]]$latent[(n_cells + 2):(2 * n_cells + 1)]  # ICAR random year slopes
  #   random_effects_kappa <- samp1[[i]]$latent[(2 * n_cells + 2):(2 * n_cells + 1 + n_route)]  # IID random route intercepts
  # 
  ...
  post_sum <- data.frame(alpha_i=cells_with_counts,
                         ...
                         tau, tau_ll, tau_ul, tau_iw, tau_sig=NA, tau_prob0)
